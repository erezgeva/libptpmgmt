...
build() {
  # PHP is excluded as it does not have a system include path
  ./configure --prefix=/usr --sbindir=\${prefix}/bin
  make -j PMC_USE_LIB=so --no-print-directory all doxygen
  # Place here as make do not use packages!
  make -j DESTDIR=install install --no-print-directory
}
