...
class ChronyThreadSet
{
  private:
    int m_timeBaseIndex;
    string m_udsAddrChrony;
    thread m_self;
    ptp_event &m_ptp4lEvent;
    vector<sessionId_t> m_subscribedClients;
  ...
  public:
    ...
    void monitor_chronyd();
    void subscribe_chrony(sessionId_t sessionId);
    bool remove_chrony_subscriber(sessionId_t sessionId);
};
...
void ChronyThreadSet::subscribe_chrony(sessionId_t sessionId)
{
    unique_lock<rtpi::mutex> local(subscribedLock[m_timeBaseIndex]);
    m_subscribedClients.push_back(sessionId);
}

bool ChronyThreadSet::remove_chrony_subscriber(sessionId_t sessionId)
{
    unique_lock<rtpi::mutex> local(subscribedLock[m_timeBaseIndex]);
    for(auto it = m_subscribedClients.begin(); it != m_subscribedClients.end();
        ++it) {
        if(*it == sessionId) {
            // sessionId found, remove it
            m_subscribedClients.erase(it);
            return true;
        }
    }
    return false;
...
void ChronyThreadSet::chrony_notify_client()
{
    ...
    for(auto it = m_subscribedClients.begin(); it != m_subscribedClients.end();
        ...
        pmsg->setTimeBaseIndex(m_timeBaseIndex);
...
chrony_err ChronyThreadSet::subscribe_to_chronyd()
{
    ...
    m_ptp4lEvent.chrony_reference_id =
    ...
    m_ptp4lEvent.polling_interval =
        pow(2.0, interval) * 1000000;
    PrintDebug("CHRONY polling_interval = " +
        to_string(m_ptp4lEvent.polling_interval) + " us");
    ...
    m_ptp4lEvent.chrony_offset = (int)second;
    PrintDebug("CHRONY master_offset = " +
        to_string(m_ptp4lEvent.chrony_offset));
...
void ChronyThreadSet::monitor_chronyd()
{
    ...
    int fd = chrony_open_socket(m_udsAddrChrony.c_str());
    if(chrony_init_session(&m_s, fd) == CHRONY_OK && fd > 0)
        PrintInfo("Connected to Chrony at " + m_udsAddrChrony);
    for(;;) {
        if(subscribe_to_chronyd() != CHRONY_OK) {
            chrony_deinit_session(m_s);
            m_ptp4lEvent.chrony_reference_id = 0;
            m_ptp4lEvent.polling_interval = 0;
            m_ptp4lEvent.chrony_offset = 0;
            chrony_notify_client();
            PrintError("Failed to connect to Chrony at " + m_udsAddrChrony);
            // Reconnection loop
            for(;;) {
                PrintInfo("Attempting to reconnect to Chrony at " +
                    m_udsAddrChrony);
                fd = chrony_open_socket(m_udsAddrChrony.c_str());
                if(fd < 0) {
                    sleep(5); // Wait before retrying
                    continue;
                }
                if(chrony_init_session(&m_s, fd) == CHRONY_OK) {
                    PrintInfo("Reconnected to Chrony at " + m_udsAddrChrony);
                    break;
                }
            }
        }
        // Sleep duration is based on chronyd polling interval
        usleep(m_ptp4lEvent.polling_interval);
    }
}
...
ChronyThreadSet::ChronyThreadSet(int timeBaseIndex,
    const string &udsAddrChrony) :
    m_timeBaseIndex(timeBaseIndex), m_udsAddrChrony(udsAddrChrony),
    m_ptp4lEvent(ptp4lEvents[timeBaseIndex])
{
    subscribedLock[m_timeBaseIndex]; // Make dure mutex exist before thread start
    m_self = thread(thread_start, this);
}
...
int ConnectChrony::subscribe_chrony(int timeBaseIndex, sessionId_t sessionId)
{
    if(chronyThreadList.count(timeBaseIndex) > 0)
        ...
        chronyThreadList[timeBaseIndex]->subscribe_chrony(sessionId);
    else
...
int ConnectChrony::remove_chrony_subscriber(sessionId_t sessionId)
{
    for(auto &it : chronyThreadList)
        if(it.second->remove_chrony_subscriber(sessionId))
            break;
    return 0;
}
