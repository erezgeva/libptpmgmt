...
class myDisp(MessageDispatcher):
  def __init__(self):
    MessageDispatcher.__init__(self)
    self.priority1 = 0
    self.func = 0
    self.id = ''
...
class TestPtpmgmtMessageDispBuild(unittest.TestCase):
  ...
  def test_parsedCallHadlerTLV(self):
    ...
    tlv = PRIORITY1_t()
    tlv.priority1 = 117
    self.disp.callHadler(self.msg, PRIORITY1, tlv)
  ...
  def test_parsedCallHadlerTLVNoCallback(self):
    ...
    tlv = PRIORITY2_t()
    tlv.priority2 = 117
    self.disp.callHadler(self.msg, PRIORITY2, tlv)
