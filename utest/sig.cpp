...
/** @file
 * @brief Signals structures unit test
 *
...
TEST_F(SigTest, FilterWithOrgTlvs)
{
    ...
    EXPECT_TRUE(a.allowSigTlvs[ORGANIZATION_EXTENSION]);
    EXPECT_TRUE(a.allowSigTlvs[ORGANIZATION_EXTENSION_PROPAGATE]);
    EXPECT_TRUE(a.allowSigTlvs[ORGANIZATION_EXTENSION_DO_NOT_PROPAGATE]);
    EXPECT_EQ(a.allowSigTlvs.size(), 3);
    ...
    EXPECT_TRUE(a.isSigTlv(ORGANIZATION_EXTENSION_DO_NOT_PROPAGATE));
    EXPECT_TRUE(a.allowSigTlvs[ORGANIZATION_EXTENSION]);
    EXPECT_EQ(a.allowSigTlvs.count(ORGANIZATION_EXTENSION_PROPAGATE), 0);
    EXPECT_TRUE(a.allowSigTlvs[ORGANIZATION_EXTENSION_DO_NOT_PROPAGATE]);
    EXPECT_EQ(a.allowSigTlvs.size(), 2);
    EXPECT_TRUE(updateParams(a));
    ASSERT_EQ(doParse(), MNG_PARSE_ERROR_SIG);
    EXPECT_TRUE(isLastMsgSig());
    EXPECT_EQ(getSigTlvsCount(), 2);
