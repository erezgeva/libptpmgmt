...
#define startSig\
    size_t len, curLen = start(buf, sizeof buf);\
    uint16_t *cur = (uint16_t *)(buf + curLen)
#define addTlv(type, data) do { \
        len = addTlv_f(cur, type, data, sizeof data);\
        curLen += len; cur += len >> 1; } while(0)

static uint8_t buf[100];

BufSig get2MngTlvsSig()
{
    startSig;
