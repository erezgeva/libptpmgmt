...
static uint8_t dgData[] = {0xdc, 0xae, 0x14, 0xc5, 0x36, 0x59, 0x91, 0x53, 0xa8,
...
TEST(hmacTest, SHA256_128)
{
    ...
    Binary mac(16);
    EXPECT_TRUE(hmac->digest(dgData, sizeof dgData, mac));
    ...
    EXPECT_TRUE(hmac->verify(dgData, sizeof dgData, mac));
    delete hmac;
    EXPECT_EQ(hmac_count(), 0);
}
