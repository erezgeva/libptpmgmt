...
#include "ids.h"
    ...
    static void ptpmgmt_tlv_mem_free(ptpmgmt_tlv_mem self)
    {
        if(self != nullptr) {
            free(self->tlv);
            auto n = (vector<void *> *)self->_memHndl;
            if(n != nullptr) {
                for(auto m : *n)
                    free(m);
                delete n;
    ...
    {
        ptpmgmt_PTPText_t a;
        if(self != nullptr && self->tlv != nullptr && str != nullptr && *str != 0) {
            auto n = (vector<void *> *)self->_memHndl;
            if(n != nullptr) {
    ...
    ptpmgmt_PTPText_t ptpmgmt_tlv_mem_copyString(ptpmgmt_tlv_mem self,
        ptpmgmt_PTPText_t *txt)
    {
        ptpmgmt_PTPText_t a;
        if(self != nullptr && self->tlv != nullptr && txt != nullptr &&
            txt->lengthField > 0 && txt->textField != nullptr) {
            auto n = (vector<void *> *)self->_memHndl;
            if(n != nullptr) {
                size_t len = txt->lengthField;
                char *s = (char *)malloc(len + 1);
                if(s != nullptr) {
                    n->push_back(s);
                    memcpy(s, txt->textField, len);
                    s[len] = 0;
                    a.lengthField = len;
                    a.textField = s;
                    return a;
