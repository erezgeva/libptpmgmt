...
{
    if(m_lastSig)
        for(size_t i = 0; i < m_tlvs.size(); i++) {
            auto &m = m_tlvs[i];
...
bool MessageSigTlvs::traverse(const MessageSigTlvCallback &callback) const
{
    /* Use lambda to call Class method callback */
    MessageSigTlvCallback &c = *const_cast<MessageSigTlvCallback *>(&callback);
    return m_lastSig && traverse([&c](const Message & msg, tlvType_e tlvType,
    const BaseSigTlv * tlv) { return c.callback(msg, tlvType, tlv); });
}
